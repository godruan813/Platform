...
public class PageUtil {
	...
	public static <T> List<T> getListPage(Page page,List<T> list){
		int begin=page.getBeginIndex();
		int every=page.getEveryPage();
		List<T> newList=new ArrayList<T>();
		int count=0;
		while (count<every&&(begin+count)<list.size()) {
			newList.add(list.get(begin+count));
			count++;
		}
		return newList;
	}
