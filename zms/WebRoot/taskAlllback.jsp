...
<html>
			...
			{ 
				...
				for(i=0;i<title.length;i++){
					if(obj==title[i]){
						...
						title[i].style.height="35px";
						title[i].style.lineHeight="25px";
						title[i].style.fontWeight="bold";
						title[i].style.background="peachpuff";
						title[i].style.fontSize="14px";
					}
					else{
						title[i].style.top="7px";
						title[i].style.height="25px";
						title[i].style.lineHeight="20px";
						title[i].style.fontWeight="normal";
						title[i].style.background="white";
						title[i].style.fontSize="8px";
					}
				}
  ...
  <body style="overflow:hidden">
  		...
  		<div style="font-size:0;border-bottom:1px solid #DDDDDD">
			<span class="tab" id='my' onclick="change(this)" style="border-left:#E8E9EE">所有任务</span>
			<span class="tab active" id='temp' onclick="change(this)"style="">我的任务</span>
			<span style="float:right;position:relative;top:-5px;margin-right:20px"><input id="back" type="button" value="返回" onclick="backToList()"class="btn btn-default"></span>		    	    
		</div>
		<div  class="maintable" id="my" style="display:none">
			<iframe src="${pageContext.request.contextPath}/getAllPTaskByCondition.action" name="main" style="width:100%;height:100%"></iframe>		
		</div>
		<div  class="maintable" id="temp" style="display:block">	
		<form class="form-inline definewidth m10" role="form"action="getAllCaseByCondition.action" method="post">
