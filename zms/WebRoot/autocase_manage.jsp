    			...
    			<td>
    				 ...
    				 <s:set var="isExist" value="0"></s:set>  
 					 <s:iterator value="#session.sTask" id="task" status="sta">
 					 	...
 					 	&nbsp;
 					 	<s:if test="#case.isFavo==0">
 					 		<a  data='<s:property value="#case.id"/>' onclick="addToFavo(this)" style="color:blue">添加到收藏夹&nbsp;&nbsp;</a>
 					 	</s:if>
 					 	<s:else>
  							<a  data='<s:property value="#case.id"/>' onclick="delFromFavo(this)" style="color:red">从收藏夹删除&nbsp;&nbsp;</a>
 					 	</s:else>
 					 	&nbsp;
 					 	<a>其他</a>
  					 </div>
    		...
    		<table style="width:90%">
    		     <tr>
    			 	...
    			 	<td align="center" style="text-align:center;width:80%;height:80px">
    			 		<ul class="pagination">
    			 		<li>
    			 			<a href='getAllCaseByConditionP.action?&currentPage=1'>首页</a>
    			 		</li>
    			 		<c:choose>
    			 			...
    			 			<c:otherwise>
    			 				<li><a href='getAllCaseByConditionP.action?&currentPage=${page.currentPage-1}'>上一页</a></li>
    			 			</c:otherwise>
    			 		</c:choose>
    			 		<c:choose>
    			 		...
    			 		<c:forEach var="i" begin="${start}" end="${end}" step="1">
    			 				<c:choose>
	    			 				...
	    			 				<c:otherwise>
										<li><a href='getAllCaseByConditionP.action?&currentPage=${i}'>${i}</a></li>
	    			 				</c:otherwise>
    			 				</c:choose>
						</c:forEach>
    			 		<c:choose >
    			 			...
    			 			<c:otherwise>
    			 				<li><a href='getAllCaseByConditionP.action?&currentPage=${page.currentPage+1}'>下一页</a></li>
    			 			</c:otherwise>
    			 		</c:choose>
    			 		<li><a href='getAllCaseByConditionP.action?&currentPage=${page.totalPage}'>尾页</a></li>
    			 		</ul>
    			 		</td>
    			 	</tr>
    	</table>
